...
export default function Edit({
  ...
  const onChangeWariant = useCallback(
    (val: string) =>
      setAttributes({ wariant: val as BsButtonAttributes["wariant"] }),
    [setAttributes],
  );

  const onChangeRozmiar = useCallback(
    (val: string) =>
      setAttributes({ rozmiar: val as BsButtonAttributes["rozmiar"] }),
    [setAttributes],
  );
  ...
  const onChangeIkonaPozycja = useCallback(
    ( val: string ) =>
      setAttributes( { ikonaPozycja: val as BsButtonAttributes["ikonaPozycja"] } ),
    [ setAttributes ],
  );
  ...
  return (
    <>
      ...
      <div {...blockProps} style={{ display: "inline-block", ...sekcjaStyle }}>
        <span className={ btnClass }>
          { ikonaId && ICON_LIBRARY[ ikonaId ] && ikonaPozycja === "start" && (
            <span className="blok-przycisk__ikona">
              { createElement( ICON_LIBRARY[ ikonaId ].Component, {
                color: "currentColor",
                weight: "thin",
                "aria-hidden": true,
              } ) }
          ...
          <RichText
            ...
            allowedFormats={ [] }
          />
          { ikonaId && ICON_LIBRARY[ ikonaId ] && ikonaPozycja === "end" && (
            <span className="blok-przycisk__ikona">
              { createElement( ICON_LIBRARY[ ikonaId ].Component, {
                color: "currentColor",
                weight: "thin",
