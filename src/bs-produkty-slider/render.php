...
$wybrane_produkty = array_map( 'intval', $attributes['wybraneProdukty'] ?? [] );
$liczba           = max( 2, (int) ( $attributes['liczbaProduktow'] ?? 5 ) );
$predkosc         = max( 1, (int) ( $attributes['predkosc'] ?? 4 ) );
$tekst_przycisku  = $attributes['tekstPrzycisku'] ?? 'Zobacz';
...
if ( 'wybrane' === $zrodlo ) {
	// Bez wybranych produktów świadomie nie pokazujemy nic (post__in z [0]
	// gwarantuje zero wyników — pusty post__in WP_Query po cichu zignorowałby
	// i zwrócił wszystkie produkty, czego tu nie chcemy).
	$query_args['post__in']       = $wybrane_produkty ? $wybrane_produkty : [ 0 ];
	$query_args['orderby']        = 'post__in';
	$query_args['posts_per_page'] = $wybrane_produkty ? count( $wybrane_produkty ) : 1;
} else {
	$query_args['posts_per_page'] = $liczba;
...
<div <?php echo $wrapper_attrs; ?> data-predkosc="<?php echo esc_attr( $predkosc ); ?>">
	<?php if ( $query->post_count > 1 ) : ?>
		<button
			type="button"
			class="blok-produkty-slider__nav blok-produkty-slider__nav--prev is-hidden"
	...
	<?php endif; ?>
	<div class="blok-produkty-slider__viewport">
	...
	<?php if ( $query->post_count > 1 ) : ?>
		<button
			type="button"
			class="blok-produkty-slider__nav blok-produkty-slider__nav--next"
